...
- TYPE_TEXT uses the supplied text provider: prefer the current Codex conversation through `jev-codex`; the original text API remains optional. Cache a stale retry's value only while its entire helper input is identical.
